...
const getStoredLanguage = async (): Promise<SupportedLocale | null> => {
  try {
    ...
    return storedLanguage &&
      supportedLocales.includes(storedLanguage as SupportedLocale)
...
i18n
  ...
  .then(async () => {
    ...
    console.log(
      "i18next initialized with resources:",
      Object.keys(i18n.options.resources || {})
    );
    console.log(
      "zh-cn settings:",
      i18n.getResourceBundle("zh-cn", "translation")?.settings
    );
    console.log(
      "zh-tw settings:",
      i18n.getResourceBundle("zh-tw", "translation")?.settings
    );
...
export const LanguageProvider = ({ children }: { children: ReactNode }) => {
  ...
  const changeLanguage = async (lng: SupportedLocale) => {
    ...
    const normalizedLng =
      lng.toLowerCase().startsWith("zh-cn") ||
      lng.toLowerCase().startsWith("zh-hans")
        ? "zh-cn"
        : lng.toLowerCase().startsWith("zh-tw") ||
            lng.toLowerCase().startsWith("zh-hant")
          ? "zh-tw"
          : lng;
    try {
      ...
      console.log(
        "zh-cn settings after change:",
        i18n.getResourceBundle("zh-cn", "translation")?.settings
      );
      console.log(
        "zh-tw settings after change:",
        i18n.getResourceBundle("zh-tw", "translation")?.settings
      );
