...
import { updateStepNote } from "@/redux/slices/StepsDataFinal";
...
export default function SelectedImages() {
  ...
  const { currentStep, maxStep } = useSelector(
    (state: RootState) => state.step
  );
  //   api and others
  const { client_uid, task_id }: { client_uid: string; task_id: string } =
  ...
  const {
    ...
    refetch,
  } = useGetSingleByIdQuery(client_uid, { skip: !client_uid });

  const images =
    stepData?.tasks?.flatMap((task: any) =>
    ...
    return (
      <View className="mr-4 relative">
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            dispatch(removeImage({ step: currentStep, uri: item }))
          }
        >
    ...
    <View className="flex-row items-center space-x-4  gap-5">
      <TouchableOpacity
        onPress={() =>
          router.push({
            pathname: "/Camera",
            params: { id: client_uid, stepId: task_id },
          })
        }
        style={{ width: 100, height: 100, borderRadius: 8 }}
        className="border border-dashed border-[#00B8D4] rounded-full w-20 h-20 items-center justify-center"
      >
  ...
  const updatImage = async () => {
    try {
      ...
      data.append("task", JSON.stringify(note));
          ...
          <View>
            <View className="mt-4 items-center">
              <Ionicons
                ...
                color="#ccc"
              />
              <Text className="text-[#000] font-bold text-2xl my-2 ml-2">
                No note yet
              </Text>
            </View>
            <Text className="text-gray-500 text-xs mt-1">
              Tap to edit to add description for this photo
            </Text>
          </View>
        )}
      </View>
          ...
          >
            <View className="flex-row  items-center justify-between mb-5">
              ...
              <Text className="text-gray-500 text-sm mb-4">
                Tap to select photo for editing notes
              </Text>
            </View>
            {images && (
              <FlatList
